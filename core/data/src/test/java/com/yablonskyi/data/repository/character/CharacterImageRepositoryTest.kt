...
class CharacterImageRepositoryTest {
    @Test fun givenPersistenceFailure_whenImageReplaced_thenPreservesOldImageAndRemovesReplacement() = runTest {
        ...
        val images = CharacterImageRepositoryImpl(context, repository, StandardTestDispatcher(testScheduler))
    ...
    @Test fun givenValidReplacement_whenCommitted_thenRemovesOldImageAfterSavingNewPath() = runTest {
        ...
        source.outputStream().use { bitmap.compress(Bitmap.CompressFormat.PNG, 100, it) }
        bitmap.recycle()
        CharacterImageRepositoryImpl(context, repository, StandardTestDispatcher(testScheduler)).replace(7, Uri.fromFile(source).toString())
